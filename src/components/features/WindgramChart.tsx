...
interface WindgramProps {
  ...
  siteAlt: number;
  /** Optional: 7-day hourly array for the time-column table */
  hourly?: HourlySlot[];
}

export default function WindgramChart({ windgram, siteAlt, hourly }: WindgramProps) {
  const levels = [...windgram].sort((a, b) => b.alt - a.alt); // high → low (top of chart)
  const flightHours = hourly ? hourly.filter(h => h.hour >= 9 && h.hour <= 19) : [];

  return (
    <div className="flex flex-col gap-4">

      {/* ── Vertical profile grid (noon snapshot) ───────────────────────── */}
      <div className="rounded-3xl p-5 cockpit-card-glow">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[320px] border-separate border-spacing-1">
            ...
            <tbody>
              {levels.map((w, i) => {
                const ri = levels.length - 1 - i; // reversed index for labels (low → high)
                ...
                return (
                  <tr key={w.alt}>
                    ...
                    <td className="px-1 py-0.5">
                      <div className={`rounded-lg px-3 py-2 text-center font-black text-sm ${cls}`}
                        style={{ minWidth: "64px" }}>
                    ...
                    <td className="px-1 py-0.5 w-32">
                      <div className="h-3 bg-white/5 rounded-full overflow-hidden">
                        <div className="h-full rounded-full transition-all"
                          style={{
                            width: `${Math.min(100, (w.windKmh / 50) * 100)}%`,
                            background: col,
      ...
      {/* ── Hourly wind grid (if hourly provided) ───────────────────────── */}
      {flightHours.length > 0 && (
        <div className="rounded-3xl p-5 cockpit-card-glow">
          <div className="font-black text-white text-base mb-0.5">⏱️ Vento per ora (09–19h)</div>
          <div className="text-xs text-slate-500 mb-4">Colore = intensità · Freccia = direzione</div>
            ...
            <table className="border-separate border-spacing-1 min-w-max">
              <thead>
                <tr>
                  ...
                  {flightHours.map(h => (
                    <th key={h.hour} className="text-[10px] text-slate-500 font-bold text-center pb-1 min-w-[50px]">
                  ...
                  {flightHours.map(h => {
                    ...
                    return (
                      <td key={h.hour} className="py-0.5 px-0.5">
                        <div className={`rounded-lg p-1.5 flex flex-col items-center gap-0.5 ${cls}`}>
                    ...
                    return (
                      <td key={h.hour} className="py-0.5 px-0.5">
                        <div className="rounded-lg p-1.5 text-center"
                          style={{ background: "rgba(255,255,255,0.04)" }}>
                          <span className="text-[9px] font-black" style={{ color: shearCol }}>
                            {Math.round(h.shear)}
                          </span>
                        </div>
                      </td>
          ...
          <div className="text-[10px] text-slate-600 mt-2">Shear = differenza vento 10m→80m in km/h</div>
        </div>
      )}

      <div className="text-[10px] text-slate-600 text-center pb-1">
        Vento a 10m e 80m da Open-Meteo · Quote superiori estrapolate. Per sondaggi professionali usa AROME/ECMWF.
      </div>
    </div>
  );
}
