...
export default function BriefingCard({ data, selectedHour, activeHourly, selectedDayLabel, selectedDayDate }: Props) {
  const { site, todayBestHour, thermalStrength, daily } = data;
  const today = daily[0];
  const slot = selectedHour ?? data.current;

  // Use activeHourly (selected day) for window stats
  const flightWindow = (activeHourly ?? data.hourly).filter(h => h.hour >= 9 && h.hour <= 19);
  ...
  const dayDisplayLabel = selectedDayDate
    ? (() => {
        const d = new Date(selectedDayDate + "T12:00");
        const full = d.toLocaleDateString("it-IT", { weekday: "long", day: "numeric", month: "long" });
        return selectedDayLabel === "Oggi" || selectedDayLabel === "Domani"
          ? `${selectedDayLabel} — ${full}` : full.charAt(0).toUpperCase() + full.slice(1);
      })()
  ...
  const briefText = () => {
    if (selectedHour) {
      ...
      if (h.volLabel === "GO") {
        return `${String(h.hour).padStart(2,"0")}:00 — Condizioni ideali per XC. Vento ${Math.round(h.windKmh)} km/h ${h.windDirLabel}, base cumulo ${h.thermalBase}m, vario stimato ${h.thermalMs.toFixed(1)} m/s. Termiche ${thermalStrength}.`;
      }
      if (h.volLabel === "CAUTION") {
        if (h.windKmh > 25) return `${String(h.hour).padStart(2,"0")}:00 — Attenzione: vento forte ${Math.round(h.windKmh)} km/h. Raffica ${Math.round(h.gust)} km/h. Shear ${Math.round(h.shear)} km/h. Rischio turbolenza in sottovento.`;
        if (h.cape > 400) return `${String(h.hour).padStart(2,"0")}:00 — CAPE ${Math.round(h.cape)} J/kg — rischio sovrasviluppo. Rientra prima delle ore centrali.`;
        return `${String(h.hour).padStart(2,"0")}:00 — Condizioni marginali. Volabilità ${h.volability.toFixed(1)}/10. Valuta vento e termiche locali.`;
      }
      return `${String(h.hour).padStart(2,"0")}:00 — STOP. ${h.precip > 0.1 ? "Precipitazioni in atto." : h.windKmh > site.maxWindKmh ? `Vento ${Math.round(h.windKmh)} km/h oltre limite (${site.maxWindKmh} km/h).` : "Condizioni pericolose."} Non decollare.`;
    }

    if (overallLabel === "GO") {
      return `Buone condizioni per volare a ${site.name}. Finestra ottimale ${todayBestHour ? `ore ${String(todayBestHour.hour).padStart(2,"0")}:00` : "al mattino"}. Vento medio ${avgWind} km/h, termiche ${thermalStrength}. ${goHours} ore GO disponibili.`;
    }
    if (overallLabel === "CAUTION") {
      return `Condizioni marginali a ${site.name}. Vento medio ${avgWind} km/h. Solo ${goHours} ore GO su ${flightWindow.length}. Controlla CAPE e raffiche prima del decollo.`;
    }
    return `Condizioni sfavorevoli per ${site.name}. ${avgWind > site.maxWindKmh ? `Vento ${avgWind} km/h supera il limite (${site.maxWindKmh} km/h).` : "Instabilità o precipitazioni."} Non decollare.`;
  };
    ...
    <div className="rounded-3xl p-5" style={{
      ...
      <div className="space-y-2.5">
        {[
          ...
          { label: "Temp max", val: today?.tempMax ?? 0, max: 40, color: "#fb923c", display: `${Math.round(today?.tempMax ?? 0)}°C` },
        ].map((m) => (
      ...
      <div className="mt-4 pt-3 border-t border-white/5 text-[10px] text-slate-600">
        ⚠️ Briefing generato da Open-Meteo icon_seamless. Verifica sempre le condizioni locali prima del decollo.
      </div>
    </div>
  );
}
